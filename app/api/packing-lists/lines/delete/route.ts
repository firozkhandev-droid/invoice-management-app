...
import { deletePackingListLine } from "@/lib/packing-lists/packing-list-service";
import { getTenantContextForUser } from "@/lib/organisations/membership";
import { packingListLineDeleteSchema } from "@/lib/validation/packing-lists";
...
export async function POST(request: Request) {
  ...
  const parsed = packingListLineDeleteSchema.safeParse(Object.fromEntries(await request.formData()));
  if (!parsed.success) return Response.json({ ok: false, errors: parsed.error.flatten().fieldErrors }, { status: 400 });

  await deletePackingListLine(context, parsed.data);
